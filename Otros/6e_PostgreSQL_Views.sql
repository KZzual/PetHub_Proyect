-- Vistas analíticas PostgreSQL para PetHub
-- Fecha: 2025-12-01
-- Requiere esquema previamente creado (ver 6a_PostgreSQL_Schema.sql)
...
SELECT AVG(messages_count)::NUMERIC(10,2) AS avg_messages
...
-- Etiquetas potencialmente más útiles (frecuencia > media)
...
FROM v_labels_frequency lf, stats
WHERE lf.frequency > stats.avg_freq
ORDER BY lf.frequency DESC;

-- Para materializar (opcional):
-- CREATE MATERIALIZED VIEW mv_labels_frequency AS SELECT * FROM v_labels_frequency;
-- REFRESH MATERIALIZED VIEW mv_labels_frequency;
